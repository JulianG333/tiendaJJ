...
class Cart(models.Model):
    ...
    def update_subtotal(self):
        self.subtotal = sum([ 
             cp.quantity * cp.product.print for cp in self.products_related()
         ])
        self.save()
...
def set_cart_id(sender, instance, *args, **kwards):
    if not instance.cart_id:
        instance.cart_id = str(uuid.uuid4()) 
    
def update_totals(sender, instance, action, *args, **kwards):
    if action == 'post_add' or action == 'post_remove' or action == 'post_clear': 
        instance.update_totals()

def post_save_update_totals(sender, instance, *args, **kwards):
